...
本文主体描述 V1.0 完整目标架构。截至 TB-V10-029，已落地模块化后端、MySQL 结构化存储、Markdown 同步、进程内互斥锁及进程内定时调度器；Celery Worker、Celery Beat、Redis 任务队列和 ElasticSearch 索引流程尚未接入。

当前手动同步由 `POST /api/v1/knowledge/sync` 同步执行，定时同步由 API 进程内调度器执行。这是个人开发阶段的过渡实现，不改变后续将耗时同步和索引任务迁移到独立 Worker 的目标边界。阅读后续部署图和异步时序图时，应将其视为 V1.0 目标态，而不是当前已运行组件清单。
